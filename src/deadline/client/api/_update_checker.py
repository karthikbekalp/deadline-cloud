...
def check_for_updates(
    ...
    print(f"[UpdateCheck] Platform: {platform}, integration: {integration_name}, current: {current_version}")

    # Fetch the manifest
    try:
        print(f"[UpdateCheck] Fetching manifest from {MANIFEST_URL}")
        req = urllib.request.Request(MANIFEST_URL)
        with urllib.request.urlopen(req, timeout=MANIFEST_TIMEOUT_SECONDS) as resp:
            manifest = json.loads(resp.read().decode("utf-8"))
        print("[UpdateCheck] Manifest fetched successfully.")
    except urllib.error.URLError as e:
        print(f"[UpdateCheck] Network error fetching update manifest: {e}")
        return UpdateCheckResult(
        ...
        print("[UpdateCheck] Timeout fetching update manifest")
        return UpdateCheckResult(
        ...
        print(f"[UpdateCheck] Failed to parse update manifest: {e}")
        return UpdateCheckResult(
    ...
    try:
        ...
        print(f"[UpdateCheck] Found platform data. Keys: {list(platform_data.keys())}")
    except (KeyError, TypeError) as e:
        print(f"[UpdateCheck] Platform '{platform}' not found in manifest: {e}")
        available_keys = None
        try:
            available_keys = list(manifest["DeadlineCloudSubmitter"]["versions"]["latest"].keys())
        except Exception:
            pass
        print(f"[UpdateCheck] Available platform keys: {available_keys}")
        return UpdateCheckResult(
            status=UpdateCheckStatus.PARSE_ERROR,
            current_version=current_version,
    ...
    print(f"[UpdateCheck] componentVersions keys: {list(component_versions.keys())}")
    latest_version_str = component_versions.get(integration_name)
    if latest_version_str is None:
        print(f"[UpdateCheck] Integration '{integration_name}' not found in manifest for {platform}")
        return UpdateCheckResult(
    ...
    print(f"[UpdateCheck] Latest version in manifest: {latest_version_str}")
    ...
    try:
        ...
        print(f"[UpdateCheck] Invalid version string: {e}")
        return UpdateCheckResult(
    ...
    print(
        f"[UpdateCheck] Comparison: {current_ver} vs {latest_ver}, "
        f"update_available={latest_ver > current_ver}, download_url={download_url}"
    )
