...
class UpdateAvailableDialog(QDialog):
    ...
    def __init__(
        ...
        Args:
            ...
            download_url: Optional URL to download the update
    ...
    def _build_ui(self) -> None:
        ...
        headline = QLabel(
            f"<b>{tr('Version')} {self.latest_version} {tr('of Deadline Cloud for')} {self.integration_name} {tr('submitter is now available.')}</b>"
        )
        ...
        version_label = QLabel(
            f"{tr('Current:')} {self.current_version}  →  {tr('New:')} <b>{self.latest_version}</b>"
        )
        ...
        if self.release_notes_url:
            ...
            self.release_notes_button.setStyleSheet(
                "QPushButton { color: #ec7211; text-decoration: underline; border: none; background: transparent; padding: 0; text-align: left; }"
                "QPushButton:hover { color: #f59942; }"
            )
        ...
        # Dismiss button (default style with padding)
        self.dismiss_button = QPushButton(tr("Dismiss"))
        self.dismiss_button.setStyleSheet("QPushButton { padding: 3px 12px; }")
        self.dismiss_button.clicked.connect(self.accept)
        button_layout.addWidget(self.dismiss_button)

        # Download button (orange highlight, only if URL is provided)
        if self.download_url:
            self.download_button = QPushButton(tr("Download"))
            self.download_button.setStyleSheet(
                "QPushButton { background-color: #ec7211; color: white; padding: 3px 12px; }"
            )
    ...
    def _on_download_clicked(self) -> None:
        ...
        if self.download_url:
            ...
            QMessageBox.information(
                ...
                tr(f"Please restart {self.integration_name} after installation to use the new version."),
            )
            self.accept()
